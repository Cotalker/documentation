...
- **Solución:** confirma dónde instala npm los binarios globales con `npm bin -g`, y agrega ese directorio a tu `PATH`. Como atajo, siempre puedes ejecutar la herramienta vía `npx @cotctl/cli <command>`.
...
- **Solución:** agrega `-c <profile>`. Ejecuta `cotctl profile list` para ver los nombres disponibles.

### `Profile '<name>' not found`

- **Causa:** ese perfil no existe localmente (typo, o nunca te logueaste a él).
- **Solución:** revisa `cotctl profile list`; ejecuta `cotctl login` si falta.
...
- **Solución:** ejecuta `cotctl login` de nuevo para ese entorno.

### `API Error 401`

- **Causa:** el token es inválido.
- **Solución:** reautentícate con `cotctl login`.

### `API Error 403`

- **Causa:** el usuario logueado no tiene los permisos de administración requeridos.
- **Solución:** esto es un tema de permisos de Cotalker — pídele al administrador de la empresa que le otorgue al usuario los permisos necesarios, y reinténtalo.
...
- **Solución:** revisa el `--url`. On-premise, pasa la API explícitamente con `--api-url https://api.empresa.com`.
...
- **Solución:** verifica que el `--subdomain` y la `--url` coincidan con el entorno que quieres.
...
- **Solución:** revisa la indentación (espacios, no tabs) y el formato. Ejecutar `cotctl validate -f <file>` apunta a la línea problemática.
...
- **Solución:** renombra el identifier, prefijándolo con el código del formulario (ej. `re_nombre` en vez de `nombre`).
...
- **Solución:** envuelve la lógica en `function run() { ... }` (o `async function run()`).

## Formularios: preguntas huérfanas

Esta vale la pena entenderla porque es fácil de evitar y molesta de deshacer.

- **Síntoma:** después de aplicar un formulario con `questions: []`, ya no puedes re-crear preguntas con los mismos identificadores.
...
- **Solución / prevención:** nunca apliques `questions: []` para "limpiar" un formulario. Para desactivar un formulario, pon `isActive: false` *sin* tocar la sección de preguntas — `cotctl` preserva las preguntas existentes automáticamente cuando la sección está ausente. (Recuperarse de un huérfano existente requiere limpieza en el backend, así que acá la jugada es la prevención.)
...
- Re-ejecuta el comando — muchos errores incluyen una pista precisa sobre la solución.
- Para dudas de esquema, exporta un ejemplo funcionando del mismo recurso y compara:
  `cotctl <entity> export <code> -c <profile> -o example.yaml`
- Mira la [referencia de comandos](./commands/apply.md) para las opciones y el comportamiento exacto de cada comando.
